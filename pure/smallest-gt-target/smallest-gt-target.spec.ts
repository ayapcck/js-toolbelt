import { smallestGTTarget } from './smallest-gt-target';
...
describe('Even Length', () => {
  describe('Return Value', () => {
    it('should be correct when target is on either side of mid point', () => {
      ...
      expect(smallestGTTarget(testData, testData[rightMidPoint], 0)).toBe(rightMidPoint);
      expect(smallestGTTarget(testData, testData[leftMidPoint], 0)).toBe(leftMidPoint);
    });
  });
});
describe('Odd Length', () => {
  describe('Return Value', () => {
    it('should be correct for middle element', () => {
      ...
      expect(smallestGTTarget(testData, middleValue, 0)).toBe(middleIndex);
    });
  });
});
